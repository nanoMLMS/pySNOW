...
def test_ih_100_spherical():
    ...
    """
    el, pos = read_xyz("test/ih_40_test.xyz")  # very spherical
    b, c, k = gyr_desc(pos)
...
def test_dh_100_flat():
    ...
    """
    el, pos = read_xyz("test/dh_100_test.xyz")
    b, c, k = gyr_desc(pos)
    assert 1 - k < 0.1


#the gyration tensor should only be computed in the center of positions reference system
#thus this test is not needed anymore
...
#    b2, c2, k2 = gyr_desc(1, pos)
#
#    assert np.allclose([b1, c1, k1], [b2, c2, k2])

    #would be nice to find some non-cylindrical (and non prismical) cluster to test acylindricity>>0
